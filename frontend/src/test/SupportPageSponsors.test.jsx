...
describe('SupportPage — Sponsors', () => {
  it('renders the outlined "be the first" placeholder when the roster is empty', async () => {
    ...
    expect(screen.getByText(/Be the first to sponsor MLACLabs/i)).toBeInTheDocument();
  ...
  it('renders each sponsor as an external logo link when the roster is populated', async () => {
    ...
    const acme = await screen.findByLabelText('Visit Acme Corp, a MLACLabs sponsor');
    const globex = screen.getByLabelText('Visit Globex, a MLACLabs sponsor');
